  ...
  populateBilldescrips=function(targetInternalID: String)
  {
  	this.http.get(environment.apiBaseUrl + 'api/billdescrips/' + targetInternalID +'/'+new Date().getTime()).subscribe(
  		(res: Response)=>{
  			...
  			this.TYPE=this.existingData[0].TYPE;
  			this.DESCRIPTION=this.existingData[0].DESCRIPTION;
  			this.CREATED_BY=this.existingData[0].CREATED_BY;
  			this.CREATED_WHEN=this.existingData[0].CREATED_WHEN;
  			this.UPDATED_BY=this.existingData[0].UPDATED_BY;
  			this.UPDATED_WHEN=this.existingData[0].UPDATED_WHEN;  						
  		}
  		)
  }
...
addRecords=function(data){
	...
	this.dataObj={
		...
		"UPDATED_WHEN":data.UPDATED_WHEN,
		}
